...
import { success } from '../../actions/task/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['deadline'] ? itemToLinks(item['deadline']) : ''}</td>
            <td>{item['lesson'] ? itemToLinks(item['lesson']) : ''}</td>
...
const mapStateToProps = (state) => {
  return {
    data: state.task.list.data,
    error: state.task.list.error,
    loading: state.task.list.loading,
    deletedItem: state.task.del.deleted,
  };
};
